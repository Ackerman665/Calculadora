...
function binario(){
    x= parseInt(x); 
    pantalla.innerHTML= x.toString(2);
    pantalla.innerHTML=xi.log('Binary:' + ' ' + result);
        }

function octal(){
    x= parseInt(x); 
    pantalla.innerHTML= x.toString(8);
    pantalla.innerHTML=xi.log('Octal:' + ' ' + result);
}

function hexadecimal(){
    x= parseInt(x); 
    pantalla.innerHTML=x.toString(16);  //Convertit el resultado de una operacion en valor Hexadecimal 
    pantalla.innerHTML=xi.log('Hexadecimal:' + ' ' + result);  //Mostrar el resultado en pantalla
}


function calcularSeno(){
    x=Math.sin(x);//calcular seno y se guarda en la misma variable
    pantalla.innerHTML=x; 
    xi=1;     
}

function calcularCoseno(){
    x=Math.cos(x);//calcular coseno y se guarda en la misma variable
    pantalla.innerHTML=x; 
    xi=1;
}

function calcularTangente(){
    x=Math.tan(x);//calcular tangente y se guarda en la misma variable
    pantalla.innerHTML=x; 
    xi=1;
}

function calcularLn(){
    x=Math.log(x);//calcular log y se guarda en la misma variable
    pantalla.innerHTML=x; 
    xi=1;
}
